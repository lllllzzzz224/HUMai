...
rois=[(300,260,330,290),(80,280,110,310),(250,375,280,400)]
targets={'complete_label':(218,301),'previous_target_visible_surface':(358,451),'left_truncated_visible_surface':(150,470)}
def pts(r,d,uv):
...
for name in targets:
 a=np.array([r['targets'][name]['surface_point_m'] for r in out['records']]);out[name]={'surface_median_m':np.median(a,0).tolist(),'surface_span_mm':(np.ptp(a,axis=0)*1000).tolist(),'note':'selected 11x11 visible surface patch; not full-fruit center or sphere center; no historical XY correction'}
