...
rclpy.init();n=rclpy.create_node('new_table_readonly_capture');b=CvBridge();tf=tf2_ros.Buffer();listener=tf2_ros.TransformListener(tf,n)
...
try:
 while time.time()-start<12:
  ...
  if delta>.02 or post['end_wall']-pre['start_wall']>.6:errors.append('Unstable/slow bracket');continue
 ...
 (OUT/'capture.json').write_text(json.dumps(dict(duration_s=time.time()-start,counts=counts,records=records,depth_stats=stats,errors=errors,urdf=str(URDF),urdf_sha256=hashlib.sha256(URDF.read_bytes()).hexdigest(),hand_eye=HAND.tolist(),time_policy='ROS frame epoch checked against host wall and bracketed by real controller reads; motion threshold .02deg, bracket <=.6s; no fake joint publisher'),indent=2))
 print(json.dumps(dict(counts=counts,accepted=len(records),errors=errors[-5:])));n.destroy_node();rclpy.shutdown()
