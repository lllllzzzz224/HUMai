...
from preflight_alignment import ReadOnly,load_config,P,mod
def main():
 ...
 try:
  ...
  pose=Pose();g=json.loads((P/'geometry.json').read_text());pose.position.x,pose.position.y=map(float,g['right']['surface_median_m'][:2]);pose.position.z=.17010608942352054;pose.orientation=copy.deepcopy(q)
  ...
  sources=[Path('/home/li/桌面/9月机械臂实验（算法）/reports/birdview_return_20260916/trajectory.cdr')]
  expected_hashes=['3eba8848046d03e104bd52416371f9bc1d0cbab795b0575aed07c3d718b14b09']
  ...
  for source,expected_hash in zip(sources,expected_hashes):
   ...
   elapsed+=total
  # Final small same-height correction; explicit start is verified route endpoint.
  req=GetCartesianPath.Request();req.header.frame_id=n.base;req.group_name=cfg['moveit']['group'];req.link_name=n.tcp;req.start_state.joint_state.name=traj.joint_trajectory.joint_names;req.start_state.joint_state.position=traj.joint_trajectory.points[-1].positions;req.start_state.is_diff=True;req.waypoints=[pose];req.max_step=.001;req.jump_threshold=float(cfg['cartesian']['jump_threshold']);req.revolute_jump_threshold=float(cfg['cartesian']['revolute_jump_threshold_rad']);req.avoid_collisions=True;req.max_velocity_scaling_factor=.03;req.max_acceleration_scaling_factor=.03
  f=n.cartesian_client.call_async(req);rclpy.spin_until_future_complete(n,f,timeout_sec=30);res=f.result();out['correction_fraction']=None if res is None else res.fraction;out['correction_error']=None if res is None else res.error_code.val
  if res is None or res.error_code.val!=1 or res.fraction<.999999:raise RuntimeError('final tiny correction incomplete')
  micro=res.solution.joint_trajectory
  if list(micro.joint_names)!=list(traj.joint_trajectory.joint_names):raise RuntimeError('correction joint names differ')
  out['correction_points']=len(micro.points);elapsed+=.5
  for point in micro.points:
   pp=copy.deepcopy(point);t=elapsed+point.time_from_start.sec+point.time_from_start.nanosec*1e-9;ns=round(t*1e9);pp.time_from_start.sec=ns//1000000000;pp.time_from_start.nanosec=ns%1000000000;traj.joint_trajectory.points.append(pp)
