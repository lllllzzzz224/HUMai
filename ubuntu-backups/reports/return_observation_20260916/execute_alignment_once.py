...
def main():
 ...
 record={'status':'STARTED','closed':False,'return':True};out=P/('execution_'+time.strftime('%Y%m%d_%H%M%S')+'.json')
 ...
 if approved.get('purpose')!='RETURN_TO_RECORDED_OBSERVATION_ONLY':raise RuntimeError('Wrong approved purpose')
 ...
 try:
  ...
  print('RETURN TO RECORDED OBSERVATION trajectory started',flush=True);f=handle.get_result_async();duration=approved['retime']['retimed_duration_s'];deadline=time.monotonic()+duration+10
  ...
  record['final_tcp_m']=n.tcp_position().tolist();record['status']='PASS_RETURN_TO_RECORDED_OBSERVATION_ONLY';save();print(json.dumps(record),flush=True)
