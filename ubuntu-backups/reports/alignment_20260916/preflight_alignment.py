...
spec=importlib.util.spec_from_file_location('retreat',str(P.parent/'live/single_orange_control/retreat_verify_to_scan.py'));mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod)
...
class ReadOnly(FixedScanVerify):
 ...
 def apply_protection(self):
  ...
  for i,v in enumerate(vox):s.world.collision_objects.append(box(f'alignment_observed_{i}',self.base,(v+.5)*.02,[.04,.04,.04]))
...
def main():
 ...
 try:
  ...
  pose=Pose();pose.position.x,pose.position.y,pose.position.z=.316029032,-.231046169,.17;pose.orientation=copy.deepcopy(q)
  ...
  out['observed_voxels']=n.apply_protection();n.current_state_is_valid();n.solve_ik(pose,'high alignment',attempts=3)
