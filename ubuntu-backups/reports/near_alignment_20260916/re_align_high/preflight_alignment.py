...
class ReadOnly(FixedScanVerify):
 ...
 def apply_protection(self):
  ...
  for i,v in enumerate(vox):s.world.collision_objects.append(box(f'realign_observed_{i}',self.base,(v+.5)*.02,[.04,.04,.04]))
...
def main():
 ...
 try:
  ...
  pose=Pose();g=json.loads((P/'geometry.json').read_text());pose.position.x,pose.position.y=map(float,g['right']['surface_median_m'][:2]);pose.position.z=float(p.z);pose.orientation=copy.deepcopy(q)
