...
def verify_scene(n,cfg,count):
  ...
  if len([x for x in world if x.startswith('alignment_observed_')])!=115 or len([x for x in world if x.startswith('realign_observed_')])!=count:raise RuntimeError('Observation count changed')
  ...
  record['previous_obstacles_preserved']=115
