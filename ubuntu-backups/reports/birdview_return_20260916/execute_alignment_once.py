...
def main():
 ...
 record={'status':'STARTED','closed':False,'returned_to_birdview':False};out=P/('execution_'+time.strftime('%Y%m%d_%H%M%S')+'.json')
 ...
 try:
  ...
  print('BIRDVIEW RETURN trajectory started',flush=True);f=handle.get_result_async();duration=approved['retime']['retimed_duration_s'];deadline=time.monotonic()+duration+10
  ...
  record['final_tcp_m']=n.tcp_position().tolist();record['status']='PASS_BIRDVIEW_RETURN_ONLY';record['returned_to_birdview']=True;save();print(json.dumps(record),flush=True)
