...
out['hard_stop_subscription_endpoints']=[{'node_name':x.node_name,'namespace':x.node_namespace,'type':x.topic_type,'gid':list(x.endpoint_gid)} for x in n.get_subscriptions_info_by_topic('/rm_driver/move_stop_cmd')]
out['hard_stop_publisher_endpoints']=[{'node_name':x.node_name,'namespace':x.node_namespace,'type':x.topic_type} for x in n.get_publishers_info_by_topic('/rm_driver/move_stop_cmd')]
(P/'four_step_plan/after_stop/interface_diagnosis.json').write_text(json.dumps(out,indent=2));print(json.dumps(out));n.destroy_node();rclpy.shutdown()
