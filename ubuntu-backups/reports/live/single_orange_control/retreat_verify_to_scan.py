...
def main():
    ...
    parser.add_argument('--run-log',type=Path,default=Path('/home/li/hand_eye_calibration/apple_pick_v2/results/single_apple_grasp_20260914_194844.json'))
    parser.add_argument('--config',type=Path,default=Path('/home/li/hand_eye_calibration/apple_pick_v2/single_apple_full_grasp.yaml'))
    ...
    if not np.allclose(pre-verify,[0,0,0.07],atol=1e-6):
        raise RuntimeError('Source is not a 70mm vertical retreat')
    cfg=load_config(args.config)
    if cfg['visual_xy_correction']['enabled'] or cfg['candidate_search']['yaw_offsets_deg']!=[90] or cfg['candidate_search']['pitch_stages_deg']!=[[0]]:
        raise RuntimeError('Configuration changed from yaw90/pitch0/correction-disabled')
    ...
    try:
        ...
        node.execute(up,'VERIFY->PREGRASP vertical retreat 70mm')
