...
class OrangeSegView(Node):
    ...
    def callback(self, msg):
        ...
        # Heavy-occlusion recovery:
        # 对低置信度候选附近进行一次局部放大二次检测
        recovery = []

        for d in list(detections):
            x1, y1, x2, y2, conf, pts = d

            # 只处理“已经有一点证据，但检测不稳定”的候选
            if 0.15 <= conf < 0.40:
                bw = x2 - x1
                bh = y2 - y1

                # ROI 向四周扩大，保留被叶片切开的周围橙色区域
                pad_x = int(bw * 0.65)
                pad_y = int(bh * 0.65)
                ...
                if rx2-rx1 >= 40 and ry2-ry1 >= 40:
                    ...
                    # detect() 本身使用 imgsz=960，
                    # 小 ROI 会在推理时获得更高的目标相对分辨率
                    recovered = self.detect(roi, rx1, ry1)
                    ...
                    for r in recovered:
                        ...
                        dist = ((rcx-ocx)**2 + (rcy-ocy)**2)**0.5
                        limit = 0.75 * max(bw, bh)

                        if dist < limit:
                            recovery.append(r)

        if recovery:
            detections.extend(recovery)
