...
def build_parser(default_config: Path) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--place-target",
        choices=["basin", "return"],
        default="basin",
        help="抓取后落料目标：basin（放入盆子，默认）或 return（原位放回）",
    )
    return parser
...
def resolve_run_mode(args, cfg) -> RunMode:
    if args.continuous:
        ...
        return RunMode(
            ...
            settings=settings,
            place_target=str(getattr(args, "place_target", "basin")),
        )
